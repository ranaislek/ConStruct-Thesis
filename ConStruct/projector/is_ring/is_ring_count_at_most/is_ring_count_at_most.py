...
def has_at_most_n_rings(graph, n):
    """Return True if the graph has ring count ≤ n (structural constraint)."""
...
def ring_count_at_most_projector(graph, max_rings):
    """
    Edge-Deletion Projector: Enforces structural constraint: ring count (basis size) ≤ max_rings.
...
def count_rings_at_most(graph):
    """Return the ring count (basis size) in the graph for 'at most' constraints."""
    cycles = nx.cycle_basis(graph)
    return len(cycles) 