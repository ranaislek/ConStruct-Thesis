#!/usr/bin/env python3
"""
Complete Moses Dataset Analysis Script
=====================================

This script performs comprehensive analysis of the Moses dataset including:
...
All results are saved to the moses_analysis directory structure.
"""
...
def load_moses_dataset(split="train"):
    """Load Moses dataset using ConStruct's proper loading mechanism"""
    try:
        from ConStruct.datasets.moses_dataset import MosesDataset
        
        print(f"🔍 Loading Moses dataset ({split} split) using ConStruct's dataset loader...")
...
def load_full_moses_datasets():
    """Load all Moses dataset splits"""
    try:
        from ConStruct.datasets.moses_dataset import MosesDataset
        
        print("🔍 Loading full Moses dataset (train + val + test)...")
...
def create_plots(ring_count_dist, ring_length_dist, planar_count, non_planar_count, 
    ...
    plt.title(f'Moses Dataset: Ring Count Distribution ({analysis_name})', fontsize=14, fontweight='bold')
    ...
    plt.title(f'Moses Dataset: Ring Length Distribution ({analysis_name})', fontsize=14, fontweight='bold')
    ...
    if planar_count is not None:
        ...
        plt.title(f'Moses Dataset: Planarity Distribution ({analysis_name})', fontsize=14, fontweight='bold')
    ...
    plt.title(f'Moses Dataset: Cumulative Ring Count Distribution ({analysis_name})', fontsize=14, fontweight='bold')
...
def save_results(ring_count_dist, ring_length_dist, ring_count_rates, ring_length_rates, 
    ...
    # Save as JSON
    with open(output_dir / f"moses_{analysis_name}_analysis.json", 'w') as f:
        json.dump(results, f, indent=2)
    
    # Save detailed text report
    with open(output_dir / f"moses_{analysis_name}_analysis_report.txt", 'w') as f:
        f.write(f"Moses {analysis_name.title()} Analysis Report\n")
        f.write("=" * 60 + "\n\n")
...
def create_readme(output_dir):
    ...
    readme_content = """# Moses Dataset Structural Analysis
...
def main():
    ...
    print("🔬 Complete Moses Dataset Analysis")
    print("=" * 50)
    ...
    train_dataset = load_moses_dataset("train")
    ...
    full_datasets = load_full_moses_datasets()
